...
class Point(namedtuple('Point', ['x', 'y'])):
    ...
    def neighbors8(self):
        """four neighbors (with diagonals)"""
        x, y = self
        return ((x + 1, y), (x - 1, y), (x, y + 1), (x, y - 1),
                (x + 1, y + 1), (x - 1, y - 1), (x + 1, y - 1), (x - 1, y + 1))

    def manhattan_dist(self, other=None):
        """the manhattan / taxicab / cityblock distance"""
        if not other:
            other = Point(0, 0)

        return abs(self.x - other.x) + abs(self.y - other.y)

    def euclidean_dist(self, other=None):
        """the euclidean distance, also known as the hypotenuse"""
        if not other:
            other = Point(0, 0)
...
# LINQ implementation
# source: https://gist.github.com/sharkbound/07979c90e161388c09c2f7848e550c7f#file-linq-py
class Query:
    ...
    def first(self, predicate=None, default=None):
        """returns first item that ether meets the condition or first item in value if func is None"""
        for x in self:
    ...
    def last(self, predicate=None, default=None):
        """returns last item that ether meets the condition or last item in value if func is None"""
    ...
    def at(self, index, default=None):
        ...
        return next(islice(self, index, None), default)
    ...
    def aggregate(self, func, start_value=None):
        """returns the result of the function with the result of the last call as the first parameter,
        and the next item the second"""
